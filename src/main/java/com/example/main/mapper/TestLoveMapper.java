...
public interface TestLoveMapper {
    ...
    // 2，更新数据@Update("update love set name=#{name} where id=1")
    public void updateTestUser(TestLove testUser) throws Exception;
    // 3, 新增数据@Insert("insert into love(name, days, number_meet, number_leave) values(#{name},#{days},#{number_meet},#{number_leave})")
    public void addTestUser(TestLove testUser) throws Exception;
    //4, 根据id删除数据@Delete(" delete from love where id = #{testUserNo}")
